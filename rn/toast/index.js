...
import { StyleSheet, Modal as Modal$1, Easing, Animated } from 'react-native'
...
var _excluded$1 = ['className', 'style', 'level', 'children', 'size', 'color', 'scale', 'weight']
...
function Text(props) {
  ...
  return React.createElement(
    TaroText,
    _extends({ style: _mergeEleStyles$2(_getStyle$2(textClz), textStyle) }, extraProps),
    children
  )
}
...
function useEnhancedState(initialState) {
  ...
  function setEnhancedState(record, val) {
    if (typeof record === 'string') {
      setState(_objectSpread$2(_objectSpread$2({}, state), {}, _defineProperty({}, record, val)))
    } else {
      setState(_objectSpread$2(_objectSpread$2({}, state), record))
    }
  }
