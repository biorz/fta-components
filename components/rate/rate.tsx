...
import { scale as autoFix } from '../../common'
import '../../style/components/rate/index.scss'
import { RateProps, StarProps } from '../../types/rate'
...
function Rate(props: RateProps): JSX.Element {
  ...
  const onStarClick = (index: number, left: boolean) => {
  ...
  const getStarStatus = (i: number): keyof typeof STAR => {
    const val = value! - 1
    const result = val - i
    if (result >= 0) {
    ...
    <View className={rootClass} style={rootStyle}>
      {emptyList.map((_, i) => (
        <Star
          ...
          size={size}
          gutter={gutter}
          onClick={(left) => onStarClick(i, left)}
...
function Star(props: StarProps): JSX.Element {
  ...
  if (size) {
    rootStyle.width = autoFix(size)
    rootStyle.height = autoFix(size)
  }
  if (gutter) rootStyle.marginRight = autoFix(gutter)

  const onHalfClick = (left: boolean) => {
    if (half) {
      onClick?.(left)
    }
  }
  ...
  return (
    <View className={rootClass} style={rootStyle} onClick={half ? void 0 : () => onClick?.(true)}>
      ...
      {half ? (
        <>
          <View
            className='fta-star--placeholder fta-star--left'
            onClick={() => onHalfClick!(true)}
          />
          <View
            className='fta-star--placeholder fta-star--right'
            onClick={() => onHalfClick!(false)}
          />
        </>
      ) : null}
    </View>
...
const RateDefaultProps: RateProps = {
  value: 1,
  min: 1,
  count: 5,
  half: false,
