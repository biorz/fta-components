...
function getRangesAndValuesOverIndexs(indexs, options) {
    ...
    indexs.forEach((v, i) => {
        ...
        if (i + 1 < len) {
            tmp = ((_a = tmp[v]) === null || _a === void 0 ? void 0 : _a.children) || [];
        }
    });
...
function getDepth(options) {
    ...
    while (tmp) {
        depth++;
        tmp = (_a = tmp.children) === null || _a === void 0 ? void 0 : _a[0];
    }
    return depth;
}
const Cascader = forwardRef(function Cascader(props, ref) {
    const { value, options, onChange, onConfirm, depth } = props, extraProps = __rest(props, ["value", "options", "onChange", "onConfirm", "depth"]);
    ...
    const [ranges, setRanges] = useState(emptys.map(() => []));
    const [visible, toggle] = useState(false);
    ...
    const _onChange = (newIndex, index) => {
        ...
        if (+new Date() - startRef.current < 1000)
            return;
    ...
    // }, [indexs])
    return (React.createElement(BasePicker, Object.assign({}, extraProps, { methods: methods, isOpened: visible, onClose: methods.hide, 
        // @ts-ignore
        value: values }), emptys.map((_, i) => (React.createElement(ScrollArea, { activeIndex: indexs[i], range: ranges[i] || [], key: i, onChange: (newIndex) => _onChange(newIndex, i) })))));
});
const defaultProps = {
    options: [],
    value: [],
