...
const defaultDesc = {
    empty: {
        title: '暂无数据',
        desc: '让数据再飞一会',
        src: Assets$1.empty.default,
    },
    error: {
        title: '呃，出错了…',
        desc: '刷新一下试试吧',
        src: Assets$1.empty.error,
    },
};
class Empty extends Component {
    render() {
        ...
        const { className, 
        // @ts-ignore
        style, customStyle, type, showBtn, btnText, onClick, } = this.props;
        const { src, title, desc } = Object.assign(Object.assign({}, defaultDesc[type]), this.props);
        ...
        return (React.createElement(View, { className: rootClass, style: rootStyle },
            React.createElement(Image, { className: 'fta-empty-image', src: src, mode: 'aspectFit' }),
            title ? (React.createElement(View, { className: 'fta-empty-title' },
                React.createElement(Text, { level: 5, className: titleClz }, title))) : null,
            desc ? (React.createElement(View, { className: 'fta-empty-desc' },
                React.createElement(Text, { level: 6, className: descClz }, desc))) : null,
            showBtn ? (React.createElement(FTAButton, { className: btnClz, onClick: onClick },
                React.createElement(Text, { color: '#ffffff', level: 5 }, btnText))) : null));
    }
}
Empty.defaultProps = {
    type: 'empty',
