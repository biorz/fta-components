import './index.css';
import { View, Text } from '@tarojs/components';
import React from 'react';
import { useCarelessClass, isString } from '../common';

function Tag(props) {
    const { className, 
    // @ts-ignore
    style, customStyle, type, textStyle, textClassName, bgColor, border, color, borderColor, children, onClick, } = props;
    const rootClz = useCarelessClass(['fta-tag'], [`fta-tag--${type}`, border && 'fta-tag--bordered', className]);
    const textClz = useCarelessClass(['fta-tag__text'], [`fta-tag__text--${type}`, textClassName]);
    const rootStyle = Object.assign(Object.assign({}, style), customStyle);
    const txtStyle = Object.assign({}, textStyle);
    if (bgColor)
        rootStyle.backgroundColor = bgColor;
    if (borderColor)
        rootStyle.borderColor = borderColor;
    if (color)
        txtStyle.color = color;
    return (React.createElement(View, { onClick: onClick, className: rootClz, style: rootStyle }, isString(children) ? (React.createElement(Text, { style: txtStyle, className: textClz }, children)) : (children)));
}
const defaultProps = {
    type: 'primary',
    border: true,
};
Tag.defaultProps = defaultProps;
// export default class Tag extends Component<IProps, Istate> {
//   state = {
//     isRN: Taro.getEnv() === Taro.ENV_TYPE.RN,
//   }
//   render() {
//     const {
//       mode = 'light',
//       type = 'primary',
//       text,
//       shape = 'square',
//       size = 'medium',
//       clickTag = () => {},
//       clickIcon = () => {},
//       closeable = false,
//     } = this.props
//     const { isRN } = this.state
//     return (
//       <View className={`fta-tag  fta-shape-${shape} fta-size-${size} fta-mode-${mode}-${type}`}>
//         <Text
//           onClick={(e) => clickTag(e)}
//           className={`fta-size-${size}__word fta-mode-${mode}-${type}__word`}>
//           {text}
//         </Text>
//         {closeable && !isRN ? (
//           <Text className='fta-icon fta-icon-close' onClick={() => clickIcon()}></Text>
//         ) : null}
//         {closeable && isRN ? (
//           <Image
//             className='fta-tag__icon'
//             src='https://image.ymm56.com/ymmfile/operation-biz/f45222e4-4884-43ef-ae5b-fb2bd79d7263.png'
//           />
//         ) : null}
//       </View>
//     )
//   }
// }

export { Tag as default };
